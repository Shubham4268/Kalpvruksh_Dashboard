...
class Genlib
{
  ...
  public function checkLogin()
  {
    if (empty($_SESSION['admin_id']) && empty($_SESSION['user_id'])) {
      // Redirect to login page if neither an admin nor a user is logged in
      echo '<pre>';
      print_r($_SESSION);
      echo '</pre>';
      exit;  // Stops execution after printing session data

      redirect(base_url() . '?red_uri=' . uri_string());
    }
  }
