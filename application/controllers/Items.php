...
class Items extends CI_Controller
{
  public function __construct()
  {
    ...
    $this->load->model(['item', 'Category_model', 'Manufacturer_model','Admin']);
  }
  ...
  public function add()
  {
    ...
    $this->form_validation->set_rules('itemCode', 'Item Code', ['required', 'trim', 'max_length[20]', 'is_unique[items.code]'], ['required' => "required", 'is_unique' => "There is already an item with this code"]);
    $this->form_validation->set_rules('itemCategory', 'Item Category', ['required'], ['required' => "required"]);
    $this->form_validation->set_rules('itemMnf', 'Manufacturer', ['required'], ['required' => "required"]);
    ...
    if ($this->form_validation->run() !== FALSE) {
      ...
      if ($insertedId) {
        $this->genmod->addevent("Creation of new item", $insertedId, $desc, "items", $this->session->admin_id);
      }
    ...
    */
    ...
    public function edit()
    {
        ...
        $this->form_validation->set_rules('itemName', 'Item Name', [
            'required',
            'trim',
            'callback_crosscheckName[' . $itemId . ']'
        ], ['required' => 'required']);
        $this->form_validation->set_rules('itemCode', 'Item Code', [
            'required',
            'trim',
            'callback_crosscheckCode[' . $itemId . ']'
        ], ['required' => 'required']);
        ...
        $this->form_validation->set_rules('itemMnf', 'Manufacturer', ['required', 'trim']);
        ...
        if ($this->form_validation->run() !== FALSE) {
            ...
            $itemMnf = set_value('itemMnf');
    
            // update in DB
            $updated = $this->item->edit($itemId, $itemName, $itemDesc, $itemPrice, $itemCategory, $itemMnf);
    
            $json['status'] = $updated ? 1 : 0;
    
            if ($updated) {
                $desc = "Item with code '$itemCode' was updated.";
                $this->genmod->addevent("Item Update", $itemId, $desc, 'items', $this->session->admin_id);
            }
        } else {
            $json['status'] = 0;
            $json = array_merge(['status' => 0], $this->form_validation->error_array());
        }
