...
{
    //set the sort order
    $orderBy = $this->input->get('orderBy', TRUE) ?: "transDate";
    $orderFormat = $this->input->get('orderFormat', TRUE) ?: "DESC";

    // Get the total number of transactions based on role
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === "User") {
        $user_phone = $_SESSION['user_phone'];
        $totalTransactions = $this->transaction->countUserTransactions($user_phone);
    } else {
    ...
    if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === "User") {
        $data['allTransactions'] = $this->transaction->getUserTransactionsPaginated($user_phone, $orderBy, $orderFormat, $start, $limit);
    } else {
